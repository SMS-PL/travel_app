...
const ProfileImageUploader = ({uploadingImage, setUploadingImage, imageURL, setImageURL}) => {
    ...
    const handleImageChange = async (e) => {
        ...
        if(image) {
            ...
            try {
                ...
                const storageRef = ref(storage, `images/${auth.id}/profile/${image.name}`);
    ...
    const updatePhotoUrl = async (newPhotoUrl) => {
		fetch(`http://localhost:5000/api/v1/users/${auth.id}`, {
            ...
            body: JSON.stringify({
                "profileImage": String(newPhotoUrl),
            })
		})
		.then(response => {
			if (!response.ok) {
		...
		.then(data => {
			console.log("Pomyślnie zapisano zdjęcie profilowe!");
            console.log(data);
    ...
    return (
        <div className="flex flex-row justify-center items-center absolute bottom-[14px] right-[7px] ">
            <Label
