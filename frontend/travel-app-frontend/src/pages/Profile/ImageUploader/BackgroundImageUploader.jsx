...
const BackgroundImageUploader = ({uploadingImage, setUploadingImage, imageURL, setImageURL}) => {
    ...
    const handleImageChange = async (e) => {
        ...
        if(image) {
            ...
            try {
                ...
                const storageRef = ref(storage, `images/${auth.id}/background/${image.name}`);
    ...
    const updateBackgroundPhotoUrl = (newBackgroundPhotoUrl) => {
		...
		fetch(`http://localhost:5000/api/v1/users/`, {
            ...
            body: JSON.stringify({
                "backgroundImage": String(newBackgroundPhotoUrl),
            })
		})
		.then(response => {
			if (!response.ok) {
		...
		.then(data => {
			console.log("Pomyślnie zapisano zdjęcie w tle!");
            console.log(data);
            ...
            signIn({
                ...
                userState: {
                    ...auth,
                    backgroundUrl: newBackgroundPhotoUrl
                },
            });
		})
		.catch(error => {
    ...
    return (
        <div className="flex flex-row justify-center items-center absolute bottom-[10px] right-[10px] ">
            <Label
                htmlFor="BackgroundImageUploader"
            ...
            >
                {uploadingImage ? (
                    ...
                    <Icons.camera className="h-6 w-6 fill-white"/>
                )}
            </Label>

            <Input
                id="BackgroundImageUploader"
                type="file" 
